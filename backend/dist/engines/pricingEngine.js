...
function matchLocation(coord, name, ruleGeo, ruleName, radiusKm) {
    const normRuleName = ruleName || 'Any';
    if (normRuleName.toLowerCase() === 'any')
        return true;
...
export function fleetEconomics(dbData) {
    ...
    const vehicleBreakdown = dbData.vehicles?.map((v) => {
...
export async function calculatePrice(input, env) {
    ...
    const { liveKm, deadKm, vehicleId, journeyType, originName, destinationName, originCoords, destinationCoords, waitingMins, departureDate, returnDate } = input;
    const vehicle = data.vehicles.find((v) => v.id === vehicleId);
    if (!vehicle)
        throw new Error("Vehicle not found");
    // 1. Check Route Templates (Radius Match or Exact Match)
    const template = data.routeTemplates.find(t => t.vehicleId === vehicleId &&
    ...
    if (template) {
        ...
        const matrix = data.pricingMatrix.find(m => m.vehicleId === vehicleId &&
            m.status === 'active' &&
    ...
    if (isManualQuote) {
        ...
        const vEco = eco.vehicleBreakdown.find((b) => b.id === vehicleId);
        let opDays = 1;
        if (returnDate && departureDate && new Date(returnDate) > new Date(departureDate)) {
            opDays = Math.max(1, Math.ceil((new Date(returnDate).getTime() - new Date(departureDate).getTime()) / 86400000) + 1);
        }
